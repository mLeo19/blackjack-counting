...
export function useGameController(initialBankroll?: number) {
  ...
  const [visibleBankroll, setVisibleBankroll] = useState(initial.bankroll);
  const [isAnimating, setIsAnimating] = useState(false);
  const [flyingCards, setFlyingCards] = useState<FlyingCardData[]>([]);
  const resolvedGame = useRef<GameState>(initial);
  ...
  }, [dealOneCard, resetCount, clearHistory, runSettlement]);
  ...
  const hit = useCallback(async () => {
    ...
    resetHint();
    setIsAnimating(false);
  }, [visibleGame, dealOneCard, resetHint, runSettlement]);
